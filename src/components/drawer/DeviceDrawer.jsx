...
const DeviceDrawer = ({ open, setIsOpen, getDevices, seriesData }) => {
    ...
    async function getSeries(id) {
        setSelectedBrand(id)
        try {
            const resp = await axios.get(`${import.meta.env.VITE_SERVER_URL}/api/series/getbybrand/${id}`)
            if (resp) {
                console.log("response for series", resp)
                setSeries(resp?.data?.series)
            }
        } catch (error) {
            console.log("error in getting series", error)
        }
    }
    ...
    const handleAdd = async (data) => {
        ...
        try {
            ...
            const resp = await axios.post(`${import.meta.env.VITE_SERVER_URL}/api/device/create`, values)
            if (resp) {
                getDevices()
        ...
        <Drawer anchor="right" open={open} onClose={onClose}>
            ...
            <Box sx={{ width: 480, p: 3 }}>
                <Typography variant="h6" style={{
                    marginBottom: '30px'
                }}>{seriesData ? 'Update Device' : 'Add Device'}</Typography>
                ...
                <FormControl fullWidth margin="normal" size="small">
                    ...
                    <Select
                        ...
                        onChange={(e) => getSeries(e.target.value)}
                        displayEmpty
                        sx={{
                            background: "#fff",
                            borderRadius: "8px",
                ...
                <FormControl fullWidth margin="normal" size="small">
                    <InputLabel id="category-select-label">Series</InputLabel>
                    <Select
                        labelId="category-select-label"
